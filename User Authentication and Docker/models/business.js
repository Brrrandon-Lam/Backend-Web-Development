...
async function getBusinessOwner(id) {
    const [businessOwner] = await mysqlPool.query(
        'SELECT ownerid AS owner FROM businesses WHERE id = ?',
        [id]
    );
    return businessOwner[0].owner;
}

exports.getBusinessOwner = getBusinessOwner;
